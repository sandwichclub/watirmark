...
describe '#update' do
  ...
  specify 'model update should remove empty keys' do
    ...
    -> { keys.update(':' => '') }.should_not raise_error
    -> { keys.update(nil => '') }.should_not raise_error
    -> { keys.update('   ' => '') }.should_not raise_error
    -> { keys.update('   '.to_sym => '') }.should_not raise_error
  end
end
...
describe 'methods in Enumerable should not collide with model defaults' do
  it '#zip' do
    ...
    FactoryTest::ZipModel.new.zip.should == 78_732
  end
...
describe 'keywords' do
  ...
  specify 'should be able to list keywords for a model' do
    FactoryTest::SomeModel.new.keywords.sort.should == %i[first_name middle_name last_name].sort
  end
...
describe '#hash_id' do
  ...
  specify 'should generate a hash value with a length of 8 - the default' do
    ...
    keys = test_strings.map { |_x| new_model.hash_id }
  ...
  specify 'should generate a hash value with a length of 20' do
    ...
    new_model = Watirmark::Model::Factory.new
    keys = test_strings.map { |_x| new_model.hash_id(length) }
  ...
  specify 'should generate keys with hex values' do
    ...
    new_model = Watirmark::Model::Factory.new
    keys = test_strings.map { |_x| new_model.hash_id(length, :hex) }
    keys.each do |key|
  ...
  specify 'should generate keys with alphanumeric values' do
    length = Watirmark::Configuration.instance.hash_id_length = 8
    new_model = Watirmark::Model::Factory.new
    keys = test_strings.map { |_x| new_model.hash_id(length, :alpha) }
    keys.each do |key|
      key.each_char do |char|
